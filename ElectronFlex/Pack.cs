...
namespace ElectronFlex
{
    ...
    public struct InvokeError
    {
        ...
        public InvokeError(InvokeData invokeData, string error)
        {
            Source = JsonConvert.SerializeObject(invokeData);
            Err = error;
        }
        
        public static string Error(InvokeData invokeData, string error)
        {
            var err = new InvokeError(invokeData, error);
